...
const SaleRegistration = () => {
  ...
  const [due, setDue] = useState<string | number>("");
  ...
  // 개인키 입력 핸들링
  const handlePrivKey = (e: ChangeEvent<HTMLTextAreaElement>) => {
   ...
   */
  const registerSaleInfo = async (pubKey: string, saleCA: string) => {
    setLoading(false);
  };
                ...
                <Stack width="30%">
                  <Card>
                    <Box sx={{ pt: "100%", position: "relative" }}>
                      <ImgStyle src={item.length !== 0 ? item : undefined} />
                    </Box>
                  </Card>
                </Stack>
