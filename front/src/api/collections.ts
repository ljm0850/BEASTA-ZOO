...
export const recentCollection = async (size: number) => {
  const { data } = await axios.get(`${ENDPOINT_URL}/collection/latest`, {
    params: { size },
  });
  return data;
};
...
export const myCollCount = async (wallet_address: string | null) => {
  const { data } = await axios.get(
    `${ENDPOINT_URL}/user/rank/${wallet_address}`
  );
  return data;
};
